// lib/quick-answer.ts
//
// The "⚡ Быстрый ответ" and "⚡ Спросить ещё" buttons: in-chat assistant or
// Mini App.
//
// Everyone gets the in-chat assistant, exactly as before: callback `ai` or
// `ai:more`, handled in lib/handler.ts. People on the allowlist below get the
// same ProxysAI as a Telegram Mini App instead — `${SITE_URL}/tg/support`, the
 ...
 * frontend deploy — the site must accept initData signed by THIS bot's token —
 * which has to be verified in Telegram before anyone else sees the button, so
 * widening the list is meant to be a reviewed code change.
 */
...
/** `open` — the menu button, `more` — the button under an assistant's answer. */
export type QuickAnswerAction = 'open' | 'more';

const LABELS: Readonly<Record<QuickAnswerAction, string>> = {
  open: '⚡ Быстрый ответ',
  more: '⚡ Спросить ещё',
};

const CALLBACKS: Readonly<Record<QuickAnswerAction, 'ai' | 'ai:more'>> = {
  open: 'ai',
  more: 'ai:more',
};
 ...
 */
export function quickAnswerButton(action: QuickAnswerAction, target: QuickAnswerTarget): InlineKeyboardButton {
  ...
  if (quickAnswerOpensWebApp(target.userId, target.chatId)) {
    ...
    if (url !== null) return { text, web_app: { url } };
  }
  return { text, callback_data: CALLBACKS[action] };
}
